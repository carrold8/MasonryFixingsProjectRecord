...
export default function ViewFloorMaterial({material, getMaterialData}){
    ...
    const deleteFloorMaterial = () => {
        setSending(true);
        MaintenanceAPIs.DeleteFloorMaterial(material.id)
        .then((response) => {
            if(response.status === 200){
                getMaterialData();
                setSending(false);
    ...
    const handleDelete = () => {
        if(window.confirm('Are you sure you want to delete ' + material.name + ' from Floor Materials?')){
            deleteFloorMaterial();
        }
    }
